...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> carregarImagem() async {
    ...
    if (alunoId != null && alunoId!.isNotEmpty) {
      ...
      setState(() {
        imagemUrl = 'http://18.222.56.92:8080/api/uploads/$alunoId.jpeg?t=$timestamp';
      });
    }
  }
  ...
  void _criarGrupo() {
    final TextEditingController nomeGrupoController = TextEditingController(text: 'Treino - ');
      ...
      builder: (context) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Text('Criar Grupo de Treino', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
                    ...
                    onPressed: () async {
                      ...
                      if (nomeGrupo.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Digite um nome para o grupo'), backgroundColor: Colors.redAccent),
                        );
                        return;
                      }
                      ...
                      try {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Grupo "$nomeGrupo" criado com sucesso'), backgroundColor: Colors.greenAccent.shade100),
                        );
                      } catch (e) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Erro: ${e.toString()}'), backgroundColor: Colors.redAccent.shade100),
                        );
                      }
                    },
